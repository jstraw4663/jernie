...
import { Colors, IconColors, Core } from '../design/tokens';
...
const MIN_TIMESTAMP_AGE_MS = 5 * 60 * 1000; // suppress "just now" noise
...
export function WeatherStrip({ stop, weatherData, cachedAt }: WeatherStripProps) {
  ...
  const showTimestamp = !!cachedAt && (Date.now() - cachedAt) > MIN_TIMESTAMP_AGE_MS;
      ...
      {!data ? (
        ...
        return (
          <>
            ...
            {showTimestamp && (
              <div style={{marginTop:6,textAlign:"right",fontSize:11,color:Core.textFaint}}>
