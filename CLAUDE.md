...
> Last updated: May 14, 2026 — v0.7.7
...
| `src/lib/refreshScheduler.ts` | Session-level Firestore read guard — `shouldReadFirestore/markRead/invalidate`; 90min debounce prevents redundant `getDocs()` on tab remount |
...
- **v0.7.7 shipped:** Google Places API billing optimization — two-phase enrichment (Pro-tier core 14-day TTL, Enterprise-tier reviews 60-day TTL, independent session debounce keys); trip-scoped Firestore subcollection `place_enrichment/{tripId}/places/{placeId}` (reads only current trip's docs, not all trips); `refreshScheduler` debounce 30min → 90min; confirm UI refactor — `ConfirmTimeSheet` deleted, confirm + AM/PM time input merged into `ItineraryItemDetailSheet`; time saves to `reservation_times` RTDB path on Confirm press only; blurb field removed from detail sheet; Overview `RentalCard` replaces `BookingCard` + `CardWithEdit` wrapper for rental car rows
- **v0.7.6 shipped:** navigation address accuracy; Fix Match search improvements; enrichment pipeline wiring (two-phase hook setup, `ttlField`/`sessionKey`/`extraBody` on `useFirestoreEnrichment`)
